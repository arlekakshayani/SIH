...
index_calculator.py
===================
Core Airfare Price Index Calculation Engine (Person B).
...
DISCLAIMER:
All weights and base values are prototype assumptions for demonstration and hackathon use.
They do not claim to be official MoSPI / DGCA weights.
"""
...
DEFAULT_ROUTE_WEIGHTS: Dict[str, float] = {
    ...
    "BLR-DEL": 0.35,
    "HYD-MAA": 0.20,
    "DEL-CHE": 0.40  # Fallback weight when paired with DEL-BOM
}
...
class IndexCalculatorService:
    ...
    def calculate_geometric_mean(prices: List[float]) -> float:
        """
        Calculates the Jevons geometric mean:
            GM = exp( (1 / n) * sum( ln(price_i) ) )

        Filters out zero or negative values. Raises ValueError if no valid prices exist.
        """
    ...
    def calculate_arithmetic_mean(prices: List[float]) -> float:
        """
        Calculates simple arithmetic mean for comparison/demonstration.
        """
        valid_prices = [p for p in prices if isinstance(p, (int, float)) and p > 0]
        if not valid_prices:
    ...
    def calculate_price_relative(self, current_price: float, base_price: float) -> float:
        """
        Price relative: (current_price / base_price) * base_value
        """
        if base_price <= 0:
    ...
    def _extract_observation(self, obs: Union[Dict[str, Any], Any]) -> Dict[str, Any]:
        """
        Normalizes observation from dict or ORM object.
        Supports:
        - route directly (e.g. 'DEL-BOM') or composite origin-destination (e.g. 'DEL' & 'BOM')
        - advance_days directly or departure/booking date difference
        - total_fare, fare, or price field names
        """
    ...
    def calculate_index(
        ...
        Steps:
        1. Group valid observations by route -> advance_days
        2. Compute geometric mean (or arithmetic mean) per window
        3. Calculate window price relatives against base prices
        4. Apply normalized advance-window weights to derive Route Index
        5. Apply normalized route weights to derive COMPOSITE Index
        6. Return detailed summary and list of records ready for DB storage
        """
        ...
        # Step 2, 3 & 4: Route-level Index Calculation
        for route, window_dict in grouped.items():
            ...
            for days, fares in window_dict.items():
                ...
                if not base_price or base_price <= 0:
                    # Fallback base price if not explicitly configured
            ...
            # Route Index = weighted combination of window price relatives
            route_index_val = sum(
        ...
        # Step 5: Composite Index Calculation
    ...
    def calculate_and_save_from_db(
        ...
        """
        Database Integration Bridge (Steps 9 & 10):
        1. Dynamically detects Person A's FlightPrice database model.
        2. Safely maps available fields (route, advance_days, total_fare / fare / price).
        3. Falls back gracefully to seed observations if FlightPrice table is empty or not yet present.
        4. Calculates Route-level and Composite indices.
        5. Persists calculated records into airfare_index table with duplicate-prevention upsert logic.
        """
        ...
        # Dynamically import Person A's model if available
        FlightPrice = None
        try:
        ...
        # Fallback to seed data if database has no records
        if not observations:
            try:
        ...
        # Step 10: Upsert results into airfare_index table
